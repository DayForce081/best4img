...
UPNG.encode.compress = function(bufs, w, h, ps, forGIF, forbidPlte)
{
	...
	if(ps!=0) {
		var qres = UPNG.quantize(bufs, ps, forGIF, w);  bufs = qres.bufs;
...
UPNG.quantize = function(bufs, ps, roundAlpha, imgWidth)
{	
	var imgs = [], totl = 0;
	...
	for(var ii=0; ii<imgs.length; ii++) {
		...
		var ww = imgWidth || (len >> 2);  // width in pixels
		var hh = (len >> 2) / ww;          // height in pixels
		
		// Error buffer: 4 floats per pixel (R, G, B, A)
		var errs = new Float32Array(len);
		...
		for(var y=0; y<hh; y++) {
			for(var x=0; x<ww; x++) {
				...
				var r = Math.max(0, Math.min(1, sb[i]   *(1/255) + errs[i]  ));
				var g = Math.max(0, Math.min(1, sb[i+1] *(1/255) + errs[i+1]));
				var b = Math.max(0, Math.min(1, sb[i+2] *(1/255) + errs[i+2]));
				var a = Math.max(0, Math.min(1, sb[i+3] *(1/255) + errs[i+3]));
				
				// Find nearest palette color via KD-tree traversal
				var nd = root;
				...
				var q = nd.est.q;
				var maxErr = 0.09;
                                var er = Math.max(-maxErr, Math.min(maxErr, r - q[0]));
                                var eg = Math.max(-maxErr, Math.min(maxErr, g - q[1]));
                                var eb = Math.max(-maxErr, Math.min(maxErr, b - q[2]));
                                var ea = Math.max(-maxErr, Math.min(maxErr, a - q[3]));
